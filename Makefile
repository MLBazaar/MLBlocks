...
fix-lint: ## fix lint issues using autoflake, autopep8, and isort
	find mlblocks tests -name '*.py' | xargs autoflake --in-place --remove-all-unused-imports --remove-unused-variables
	autopep8 --in-place --recursive --aggressive mlblocks tests
	isort --apply --atomic --recursive mlblocks tests
...
test-unit: ## run tests quickly with the default Python
	python -m pytest --basetemp=${ENVTMPDIR} --cov=mlblocks

...
MLBlocks could always use more documentation, whether as part of the
official MLBlocks docs, in docstrings, or even on the web in blog posts,
articles, and such.
...
Ready to contribute? Here's how to set up `MLBlocks` for local development.

1. Fork the `MLBlocks` repo on GitHub.
2. Clone your fork locally::

    $ git clone git@github.com:your_name_here/MLBlocks.git
...
2. The tests that cover a module called ``mlblocks/path/to/a_module.py`` should be
   implemented in a separated module called ``tests/mlblocks/path/to/test_a_module.py``.
